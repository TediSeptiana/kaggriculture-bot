...
class ExtendedDailyMatrixWriter:
    ...
    columns so the user can inspect seed inventory, product inventory, and
    raw market actions per day.
    ...
    def write(
        ...
        try:
            with open(out_path, "w", encoding="utf-8") as f:
                ...
                f.write("=" * 130 + "\n")
                f.write(
                    "NOTE: 'Seed Inv' and 'Product Inv' are best-effort extractions\n"
                    "      from the observation. Format: CROP:qty,CROP:qty\n"
                    "      If a column shows '-', the key was not found in the\n"
